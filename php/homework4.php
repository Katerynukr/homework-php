...
<p>-----------------------------------</p>
<p>Ex: 11.</p>
<p>Generate array with 101 element which values are from 0 to 300. All values 
    have to be unique. Make the highest value in the middle and other values 
    would decrease from it. Count two sums from of first and second part of array. 
    If the difference between sums is greater than 30 - repeat reorganization of array.</p>
<?php
$lastArray = [];
$count = 0;
$largestNumber = 0;
while($count < 102){
    $rand1 = rand(0, 300);
    if(!in_array($rand1, $lastArray)){
        $lastArray[] = $rand1;
        $count++;
    } 
}
print_r($lastArray);
foreach($lastArray as $isLarge){
    if($isLarge > $largestNumber){
        $largestNumber = $isLarge;
    }
}
echo '<br>';
echo $largestNumber;
